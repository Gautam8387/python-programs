...
def merge_zarr_datasets(datasets: List[Dict[str, Tuple[zarr.Array, List[str]]]]) -> Dict[str, Tuple[zarr.Array, List[str]]]:
    ...
    for array_type in all_array_types:
        ...
        # Recommanded Chunk size is 10MB = 50,000 features * 8 bytes * 2500 rows
        num_rows = 10000000 // (num_features*8) # 8 for int64
        if num_rows > 5000:
            num_rows = 5000
        ...
        for array_type in all_array_types:
            ...
            if array_type in dataset.keys():
                ...
                for rows in range(in_rows):
                    ...
                    feed_rows_idx.append(select_row)
                
                # Dump the data into the merged dataset -- one time for each array type -- faster than dumping for each row
                merged_arrays[array_type][feed_rows_idx, :] = feed_rows
                # If the row_hash is empty, generate a new hash
